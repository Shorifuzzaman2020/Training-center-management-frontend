...
export default function AdminAdmissionPage() {
  ...
  const updateAdmission = async (id: string, data: any) => {
    await axios.patch(
      `http://localhost:5000/api/v1/admissions/${id}/update-fee`,
      data,
    );
          ...
          <tbody>
            {currentData.map((a) => (
              <tr key={a._id} className="text-center">
                ...
                <td className="border p-2">{a.feeAmount}</td>
                ...
                <td className="border p-2">
                  <span
                    className={`px-2 py-1 rounded text-white ${
                      a.feeStatus === "paid"
                        ? "bg-green-500"
                        : a.feeStatus === "approved"
                          ? "bg-blue-500"
                          : "bg-yellow-500"
                    }`}
                  >
                    {a.feeStatus.toUpperCase()}
                  </span>
                </td>
                ...
                <td className="border p-2 space-x-2">
                  <button
                    onClick={() =>
                      setSelected({
                        ...a,
                        feeAmount: "", // 🔥 THIS LINE FIXES EVERYTHING
                      })
                    }
              ...
              <button
                onClick={() => {
                  updateAdmission(selected._id, {
                    feeAmount:
                      selected.feeAmount === "" ||
                      selected.feeAmount === undefined
                        ? undefined
                        : Number(selected.feeAmount),

                    approve: selected.feeStatus === "paid",
                  });
        ...
        <div className="fixed inset-0 bg-black/50 flex flex-col items-center justify-center">
          <div className="bg-white p-6 rounded w-[500px]">
            ...
            <div className="max-h-40 overflow-y-auto border rounded p-2">
              {selected1.payments?.length > 0 ? (
                selected1.payments.map((p: any, i: number) => (
                  <div
                    key={i}
